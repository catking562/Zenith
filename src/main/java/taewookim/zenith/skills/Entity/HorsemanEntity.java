...
public class HorsemanEntity extends Display.ItemDisplay {
    ...
    public HorsemanEntity(World world, Location loc, Player p) {
    ...
    public void postTick() {
        ...
        for(int j = 0; j<3;j++) {
            ...
            loc.getWorld().spawnParticle(Particle.REDSTONE, loc.clone().add(loc.getDirection().multiply(3.5)), 10, 0.2, 0.2, 0.2, 0, new Particle.DustOptions(Color.WHITE, 1));
            loc.getWorld().spawnParticle(Particle.REDSTONE, loc.clone().add(loc.getDirection().multiply(1.75)), 50, 1, 0.1, 1, 0, new Particle.DustOptions(Color.ORANGE, 0.5f));
            Transformation tran = dis.getTransformation();
